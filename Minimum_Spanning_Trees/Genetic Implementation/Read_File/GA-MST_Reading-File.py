...
class Gen_MST():
    ...
    def gerar_pop(self,pop_nodes):
        pop_nodes1 = []
        
	for i in range(0,self.tam_chrom):
	    a = A_Node(pop_nodes[i][0],pop_nodes[i][1])
	    a.id = i
            pop_nodes1.append(a)
        ...
        for i in range(0,self.population_size):
            ...
            embryon.nodes = copy(pop_nodes1)
	    embryon.set_fitness()
            self.population.append(embryon)
            random.shuffle(pop_nodes1)
...
#	victorious_pop = self.tournament(self.population,self.population_size/10)
	...
	victorious = []
	pop = copy(population)
	while len(victorious) < tournament_size:
	    random.shuffle(pop)
...
#	    victorious = self.tournament(victorious,len(victorious)/2)
    ...
    def	finish_him(self,min_fitness,generations,nodes):
        current_generation = 1
        self.gerar_pop(nodes)
...
if __name__ == "__main__":
    ...
    nodes = node_file.load(file_name)
    a = Gen_MST(len(nodes),100)
    a.finish_him(10,20000,nodes)
    a.population.sort()
    p = a.population[0]
    print '\nChromosome'
